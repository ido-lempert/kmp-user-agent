...
### AD-1 — Rule data is build-time-compiled Kotlin, normalized for every target's regex dialect

- **Binds:** CAP-1, CAP-2
- **Prevents:** (a) four different per-platform resource-loading mechanisms becoming a fifth thing to keep in sync; (b) the codegen author and the parser author independently inventing incompatible table shapes; (c) patterns that compile on JVM/Native silently throwing `SyntaxError` on JS.
- **Rule:** Browser/OS/device detection patterns are vendored from `uap-core`'s `regexes.yaml` (Apache-2.0, attribution per AD-6) and converted into `commonMain` Kotlin source by a Gradle code-gen task at build time — no target loads the rule set as a runtime resource. The generated table is three ordered pattern lists mirroring `uap-core`'s own documented structure (`user_agent_parsers`, `os_parsers`, `device_parsers`); each entry holds a regex plus `uap-core`'s replacement-template fields (`family_replacement`, `v1_replacement`, `v2_replacement`, etc.), evaluated in file order, first match wins per category. Because Kotlin/JS's `Regex` hardcodes the ECMAScript `u` flag and 140+ upstream patterns use escapes illegal under it (e.g. `\-`, `\!`, `\ `), the codegen task strips/normalizes such non-metacharacter backslash escapes **uniformly for all targets** (a safe no-op on JVM/Native) so every target compiles the identical generated pattern set.

### AD-2 — One symmetric data model for parse and generate, with a fixed concrete shape

- **Binds:** CAP-1, CAP-2
- **Prevents:** The parse direction and the generate direction independently settling on incompatible shapes (nested vs. flat, differing field types).
- **Rule:** `UserAgentInfo(browser: Component?, engine: Component?, os: Component?, device: Device?)` with `Component(name: String?, version: String?)` and `Device(brand: String?, model: String?, name: String?)`. Version stays a plain `String?` (no parsed semver) for v1. This one class is both `UserAgentParser`'s output and `UserAgentGenerator`'s input; neither direction defines a parallel model.

### AD-3 — Stateless public API, one canonical entry point per direction

- **Binds:** CAP-1, CAP-2, CAP-3
- **Prevents:** A target-specific cache/singleton changing behavior between platforms, or two independently-built call sites binding to two different spellings of the same operation.
- **Rule:** The public surface is exactly `UserAgentParser.parse(String): UserAgentInfo` and `UserAgentGenerator.generate(UserAgentInfo): String` — no coexisting `toUserAgentString()` extension or second entry point. No mutable shared state and no caching beyond the read-only compiled rule table from AD-1.
